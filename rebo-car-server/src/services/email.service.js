...
const sendEmailResetPassword = async (email, subject, contentReplacements) => {
    const htmlTemplatePath = path.join(__dirname, '..', 'assets', 'html', 'forgotPassword.html');
    readHTMLFile(htmlTemplatePath, async (err, html) => {
        if (err) {
            throw new BadRequestError(err.message);
        }
...
module.exports = { sendEmailToVerifyAccount, sendEmailOrderConfirm, sendEmailResetPassword };
