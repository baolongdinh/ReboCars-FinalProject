<template>
    <b-modal v-model="showAddDiscountModal" title="Discount Detail" hide-footer hide-header :scrollable="true">
        ...
        <b-row class="float-right my-2">
            ...
            <b-col>
                <b-button v-if="acceptConfirmBtn" variant="primary" @click="ConfirmUpdated">Confirm</b-button>
...
export default {
    ...
    methods: {
        ...
        ConfirmUpdated() {
            ...
            discountsRepo.addDiscount(payload).then((result) => {
                console.log({ result })
                this.closeModal()
                this.resetModal()
                this.$notify({
                    title: 'Notification',
                    text: 'add new discount successfully.',
                    type: 'success'
                });
    ...
    computed: {
        getAllDiscountProperty() {
            return `${this.discount_name}|${this.discount_code}|${this.discount_value}|${this.discount_description}|${this.image}`;
        }
    },
    watch: {
        getAllDiscountProperty() {
            if (this.discount_name && this.discount_code && this.discount_value && this.discount_description && this.image) {
                this.acceptConfirmBtn = true
            } else {
                this.acceptConfirmBtn = false
            }
